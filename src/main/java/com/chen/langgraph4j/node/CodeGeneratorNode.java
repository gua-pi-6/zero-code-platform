...
import com.chen.langgraph4j.model.QualityResult;
...
/**
 * 代码生成节点
 * 负责根据智能路由选择的代码生成类型，调用 AI 生成代码并保存到指定目录
 */
@Slf4j
public class CodeGeneratorNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            // 获取 AI 代码生成外观服务
            AiGenerateFacade codeGeneratorFacade = SpringContextUtil.getBean(AiGenerateFacade.class);
            log.info("开始生成代码，类型: {} ({})", generationType.getValue(), generationType.getText());
            // 先使用固定的 appId (后续再整合到业务中)
            Long appId = 0L;
            // 调用流式代码生成
            Flux<String> codeStream = codeGeneratorFacade.generateWithSaveStream(userMessage, generationType, appId);
            // 同步等待流式输出完成
